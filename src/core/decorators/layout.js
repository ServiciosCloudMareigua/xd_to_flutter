...
class Layout extends AbstractDecorator {
	...
	_pin(nodeStr, ctx) {
		...
		return "Pinned.fromSize(" +
			`bounds: Rect.fromLTWH(${$.fix(bounds.x) / artboard.width}* screenSize.width, ${$.fix(bounds.y) / artboard.height}* screenSize.height, ${$.fix(bounds.width) / artboard.width}* screenSize.width, ${$.fix(bounds.height) / artboard.height}* screenSize.height), ` +
			`size: Size(${$.fix(size.width) / artboard.width}* screenSize.width, ${$.fix(size.height) / artboard.height}* screenSize.height), ` +
	...
	_translate(nodeStr, ctx) {
		...
		return "Positioned(" +
			`left:${xStr / artboard.width}* screenSize.width, ` +
			`top:  ${yStr / artboard.height}* screenSize.height,` +
			`child: ${nodeStr},` +
			")";
		// }else{
