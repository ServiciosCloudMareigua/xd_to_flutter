...
class Text extends AbstractNode {
	...
	_getText(ctx) {
		...
		// if(this.xdNode.name.startsWith('icon:')){
		...
		// }	
		return this.xdNode.name.startsWith('icon:') ? 
		`Icon(Icons.${this.xdNode.name.substring('icon:'.length).trim()}, 
		s${this._getTextStyleParamList(this.xdNode, false, ctx)[1].substring(5)}
		${this._getTextStyleParamList(this.xdNode, false, ctx)[2]}
		)` 
		:"Text(" +
			`${text}, ` +
