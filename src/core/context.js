...
class Context {
	...
	addComponentInstance(node) {
		trace(`Context addComponentInstance node.xdNode.name ${node.xdNode.name}`);
		let instances = this.componentInstances[node.symbolId];
		trace(`Context addComponentInstance instances ${instances}`);
		if (!instances) { 
			trace(`Context addComponentInstance INSTANCES ARE UNDEFINED}`);
		...
		for (let i = 0; i < instances.length; ++i) {
			if (instances[i].xdNode === node.xdNode) {
				trace(`Context addComponentInstance IT IS ALREADY IN THE INSTANCE LIST}`);
				return;
			}
		}
		instances.push(node);
		trace(`Context addComponentInstance this.componentInstances[node.symbolId] ${this.componentInstances[node.symbolId]}`);
		if (node.isMaster) {
			this.masterComponents[node.symbolId] = node;
			trace(`Context addComponentInstance this.masterComponents[node.symbolId] ${this.masterComponents[node.symbolId].xdNode.name}`);
			this.widgetNameSet[node.widgetName] = true;
		}
	}
